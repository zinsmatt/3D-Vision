...
class PoseSE3LieAlgebra
{
    ...
    bool operator() (const T* const T1, const T* const T2,
                     T *residuals) const
    {
        Eigen::Map<const Sophus::SE3<T>> Rt1(T1);
        Eigen::Map<const Sophus::SE3<T>> Rt2(T2);

        Eigen::Matrix<T, 6, 1> res = (T12_.inverse().cast<T>() * Rt1.inverse() * Rt2).log();
        for (int i = 0; i < 6; ++i)
            residuals[i] = res[i] * T(sqrt_information_(i, i));
...
int main(int argc, char **argv) {
    ...
    while (!fin.eof()) {
        ...
        if (name == "VERTEX_SE3:QUAT") {
            ...
            auto* cost_function = new ceres::AutoDiffCostFunction<PoseSE3LieAlgebra, 6, 7, 7>(functor);
    ...
    std::ofstream fout("result_lie_ceres_autodiff.g2o");
    for (auto it : poses)
    {
